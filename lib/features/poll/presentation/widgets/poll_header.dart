      ...
      child: Row(
        children: [
          ...
          if (poll.allowMultipleVotes) ...[
            ...
            Text('• Multiple choice', style: theme.textTheme.labelSmall),
          ],
          if (!poll.isActive) ...[
            const SizedBox(width: 4),
            Icon(Icons.lock_outline, size: 14, color: theme.textTheme.bodySmall?.color),
            const SizedBox(width: 2),
            Text(poll.isClosed ? 'CLOSED' : 'EXPIRED', style: theme.textTheme.labelSmall),
          ],
          const Spacer(),
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                if (poll.isActive && onClose != null)
                  const PopupMenuItem(
                    value: 'close',
                    child: Row(children: [Icon(Icons.lock_outline, size: 18), SizedBox(width: 12), Text('Close poll')]),
                  ),
                if (onDelete != null)
                  const PopupMenuItem(
                    value: 'delete',
                    child: Row(
                      children: [Icon(Icons.delete_outline, size: 18), SizedBox(width: 12), Text('Delete poll')],
                    ),
                  ),
              ],
            ),
