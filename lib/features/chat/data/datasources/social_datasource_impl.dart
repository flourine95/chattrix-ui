...
class SocialDatasourceImpl implements SocialDatasource {
  ...
  Future<List<BirthdayModel>> getBirthdaysToday() async {
    try {
      final response = await dio.get(ApiConstants.birthdaysToday);
  ...
  Future<SendBirthdayWishesResponse> sendBirthdayWishes({required SendBirthdayWishesRequest request}) async {
    try {
      final response = await dio.post(ApiConstants.sendBirthdayWishes, data: request.toJson());
  ...
  Future<AnnouncementModel> createAnnouncement({
    ...
    try {
      final response = await dio.post(ApiConstants.announcements(conversationId), data: request.toJson());
  ...
  Future<List<MutualGroupModel>> getMutualGroups({required int userId}) async {
    try {
      final response = await dio.get(ApiConstants.mutualGroups(userId));
